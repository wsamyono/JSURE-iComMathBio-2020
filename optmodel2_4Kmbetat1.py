...
# Column 2 = input (u)
# Column 3 = output (yp)
data = np.loadtxt('dataHileymit1.txt',delimiter=',')
# u0 = data[0,1]
# yp0 = data[0,1]
# t = data[:,0].T - data[0,0]
t = data[:,0]
...
def fopdt(y,t,x,Km1,Km2,Km3,Km4,beta):
  ...
  #  try:
    if t <= 2:
                Km1 = x[0]
                dydt = Km1*y**beta-Km4*y
    else: 
        if t <= 4:
                Km2 = x[1]
                dydt = Km2*y**beta-Km4*y 
        else:
                Km3 = x[2]
                dydt = Km3*y**beta-Km4*y
  #  except:
...
plt.title('Best Fit Model without Treatment - Mendelsohn',loc='center')
# plt.legend(loc='best')
#plt.subplot(2,1,2)
