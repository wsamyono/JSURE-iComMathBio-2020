...
def fopdt(y,t,x,Km1,Km2,Km3,Km4,Km5,beta):
  ...
  #  try:
    if t <= 2.0:
                Km1 = x[0]
                dydt = Km1*y**beta-Km5*y
    else: 
        if t <= 3.0:
                Km2 = x[1]
                dydt = Km2*y**beta-Km5*y 
        else: 
             if t <= 4.0:
                Km3 = x[2]
                dydt = Km3*y**beta-Km5*y 
             else:
                Km4 = x[3]
                dydt = Km4*y**beta-Km5*y
  #  except:
...
plt.title('Best Fit Model with Treatment Data - Mendelsohn')
...
np.savetxt('outputdatamodel2treatment5um.txt',data,delimiter=',')
plt.savefig('outputmodel2treatment5um.png',dpi=300, bbox_inches='tight') 
