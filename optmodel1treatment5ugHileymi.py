...
def fopdt(y,t,x,Km1,Km2,Km3,Km4,Km5):
    # arguments
    #  y      = output
    #  t      = time
...
def sim_model(x):
 ...
 #   beta = x[5]
  ...
  #              y1 = odeint(fopdt,ym[i],ts,args=(uf,Km,taum,thetam))
        y1 = odeint(fopdt,ym[i],ts,args=(x,Km1,Km2,Km3,Km4,Km5))
        ym[i+1] = y1[-1]
    return ym
...
plt.title('Best Fit Model with Treatment Data - Exponential')
...
np.savetxt('outputdatamodel1treatment5um.txt',data,delimiter=',')
plt.savefig('outputmodel1treatment5um.png',dpi=300, bbox_inches='tight') 
